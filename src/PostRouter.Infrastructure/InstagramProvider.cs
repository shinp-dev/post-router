...
internal sealed class InstagramApiClient(HttpClient httpClient)
{
    ...
    public async Task<InstagramShortLivedToken> ExchangeCodeAsync(string appId, string appSecret, string code, Uri redirectUri, CancellationToken cancellationToken)
    {
        ...
        using var response = await SendAsync(request, "instagram_code_exchange", cancellationToken).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode) throw Rejected(response, "instagram_code_exchange");
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(accessToken))
                throw new InstagramProviderException("instagram_code_exchange_token_missing");
            var permissions = ReadPermissions(value);
            if (string.IsNullOrWhiteSpace(permissions))
                throw new InstagramProviderException("instagram_permissions_response_missing");
            return new(accessToken, permissions);
        }
        catch (JsonException) { throw new InstagramProviderException("instagram_code_exchange_response_invalid"); }
    }
    ...
    public async Task<InstagramTokenResponse> ExchangeLongLivedAsync(string appSecret, string accessToken, CancellationToken cancellationToken)
    {
        ...
        using var response = await SendAsync(request, "instagram_long_token_exchange", cancellationToken).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode) throw Rejected(response, "instagram_long_token_exchange");
        return await ReadTokenAsync(response, "instagram_long_token", cancellationToken).ConfigureAwait(false);
    }
    ...
    public async Task<InstagramTokenResponse> RefreshAsync(string accessToken, CancellationToken cancellationToken)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, uri);
        using var response = await SendAsync(request, "instagram_refresh", cancellationToken).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode) throw Rejected(response, "instagram_refresh");
        return await ReadTokenAsync(response, "instagram_refresh_token", cancellationToken).ConfigureAwait(false);
    }
    ...
    public async Task<InstagramIdentity> GetIdentityAsync(string accessToken, CancellationToken cancellationToken)
    {
        ...
        using var response = await SendAsync(request, "instagram_identity", cancellationToken).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode) throw Rejected(response, "instagram_identity");
        try
        {
            using var document = await JsonDocument.ParseAsync(await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false), cancellationToken: cancellationToken).ConfigureAwait(false);
            var value = FirstDataOrRoot(document.RootElement);
            ...
            if (string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(username))
                throw new InstagramProviderException("instagram_identity_response_invalid");
            return new(id, username);
        }
        catch (JsonException) { throw new InstagramProviderException("instagram_identity_response_invalid"); }
    }

    private static async Task<InstagramTokenResponse> ReadTokenAsync(HttpResponseMessage response, string stage, CancellationToken cancellationToken)
    {
        try
        {
            using var document = await JsonDocument.ParseAsync(await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false), cancellationToken: cancellationToken).ConfigureAwait(false);
            var value = FirstDataOrRoot(document.RootElement);
            var accessToken = ReadString(value, "access_token");
            if (string.IsNullOrWhiteSpace(accessToken)) throw new InstagramProviderException($"{stage}_missing");
            var expiresIn = value.TryGetProperty("expires_in", out var expiry) ? expiry.ValueKind switch
            {
                JsonValueKind.Number when expiry.TryGetInt64(out var number) => number,
                JsonValueKind.String when long.TryParse(expiry.GetString(), NumberStyles.None, CultureInfo.InvariantCulture, out var number) => number,
                _ => 0,
            } : 0;
            if (expiresIn <= 0) throw new InstagramProviderException($"{stage}_expiry_invalid");
            return new(accessToken, expiresIn);
        }
        catch (JsonException) { throw new InstagramProviderException($"{stage}_response_invalid"); }
    }

    private static InstagramProviderException Rejected(HttpResponseMessage response, string stage) =>
        new(response.StatusCode switch
        {
            HttpStatusCode.BadRequest => $"{stage}_bad_request",
            HttpStatusCode.Unauthorized => $"{stage}_unauthorized",
            _ => $"{stage}_rejected",
        });
    ...
    private async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, string stage, CancellationToken cancellationToken)
    {
        ...
        catch (HttpRequestException) { throw new InstagramProviderException($"{stage}_transport_failure"); }
        catch (IOException) { throw new InstagramProviderException($"{stage}_transport_failure"); }
        catch (InvalidOperationException) { throw new InstagramProviderException($"{stage}_transport_failure"); }
        catch (TaskCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            throw new InstagramProviderException($"{stage}_transport_timeout");
        }
    }
    ...
    {
        if (root.ValueKind != JsonValueKind.Object || !root.TryGetProperty("data", out var data)) return root;
    ...
    private static string? ReadPermissions(JsonElement value)
    {
        if (value.ValueKind != JsonValueKind.Object || !value.TryGetProperty("permissions", out var permissions)) return null;
        return permissions.ValueKind switch
        {
            JsonValueKind.String => permissions.GetString(),
            JsonValueKind.Array when permissions.EnumerateArray().All(item => item.ValueKind == JsonValueKind.String) =>
                string.Join(',', permissions.EnumerateArray().Select(item => item.GetString())),
            _ => null,
        };
    }
}
